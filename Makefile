...
HTTPS_GIT := github.com/nftcosmod/nftcosmoverse.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=nftcosmoverse \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=nftcosmod \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/nftcosmod ./cmd/nftcosmod
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/nftcosmod.exe ./cmd/nftcosmod
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/nftcosmod
...
draw-deps:
	...
	@goviz -i ./cmd/nftcosmod -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c nftcosmoverse --local -f chains.yaml
endif
...
set-testnet-configs:
	nftcosmod config set client chain-id chainid-1
	nftcosmod config set client keyring-backend test
	nftcosmod config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | nftcosmod keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | nftcosmod keys add acc1 --recover`
